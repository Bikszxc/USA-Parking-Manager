...
from logic.models import new_car_owner, get_car_owners, get_owner_vehicles, record_found, get_owner, get_vehicle_type, \
    park_vehicle, get_parking_slots, unpark_vehicle, get_parkslot_info, assign_vehicle, check_registration, renew_vehicle, check_plate_number, \
    get_vehicles, get_vehicle_owner
...
class DashboardScreen(tk.Frame):
    ...
    def show_frame(self):
        self.frames = [self.page_home, self.page_reservations, self.page_vehicles, self.page_accounts]
    ...
    def home_page(self):
        ...
        def car_list_ui():
            ...
            tk.Frame(frame_car_list, bg='#b80000', height=50).grid(row=0, column=0, sticky='w')
        ...
        def park_vehicle_ui():
            ...
            all_owners = sorted([name[1] for name in get_car_owners()])
            owners_with_vehicles = []

            for owner in all_owners:
                if len(get_owner_vehicles(owner)) > 0:
                 owners_with_vehicles.append(owner)
            ...
            dropdown_owner = ttk.Combobox(frame_park_vehicle, values=owners_with_vehicles, state="normal",
    ...
    def vehicles_page(self):
        ...
        def vehicle_list_ui():
            ...
            def load_vehicles():
                for row in tree_vehicles.get_children():
                    tree_vehicles.delete(row)
                vehicles = get_vehicles()
                for vehicle in vehicles:
                    owner_name = get_vehicle_owner(vehicle[1])
                    tree_vehicles.insert("", "end", values=(vehicle[0], owner_name, vehicle[2], vehicle[3], vehicle[4], vehicle[5]))

            tk.Frame(self.page_vehicles, bg='#e6e6e6', width=20).grid(row=1, column=1, sticky='nsew')

            frame_vehicle_list = tk.Frame(self.page_vehicles, bg='#b80000', padx=10, pady=10, width=600)
            frame_vehicle_list.grid(row=0, column=2, rowspan=3, sticky='nsew')

            tk.Frame(frame_vehicle_list, bg='#b80000', height=50).grid(row=0, column=0, sticky='nsew')

            label_vehicle_list = tk.Label(frame_vehicle_list, bg='#b80000', fg="white", text="Vehicle List", font=self.master.header_font)
            label_vehicle_list.place(x=0, y=0)

            tree_vehicles = ttk.Treeview(frame_vehicle_list,
                                         columns=("ID", "Owner Name", "Plate Number", "Vehicle Type",
                                                  "Registration Date", "Expiration Date"),
                                         show="headings")

            table_style = ttk.Style()
            table_style.theme_use("clam")
            table_style.configure("Treeview.Heading", font=self.master.subheader_font, background="#b80000",
                                  foreground="white", relief="flat")

            table_style.configure("Treeview", font=self.master.login_font, rowheight=25, background="#e6e6e6", fieldbackground="#e6e6e6")
            table_style.map("Treeview", background=[("selected", "#3399ff")], foreground=[("selected", "white")])

            tree_vehicles.heading("#1", text="ID", anchor="w")
            tree_vehicles.heading("#2", text="Owner Name", anchor="w")
            tree_vehicles.heading("#3", text="Plate Number", anchor="w")
            tree_vehicles.heading("#4", text="Vehicle Type", anchor="w")
            tree_vehicles.heading("#5", text="Registration Date", anchor="w")
            tree_vehicles.heading("#6", text="Expiration Date", anchor="w")

            tree_vehicles.column("#1", width=35, stretch=tk.NO)
            tree_vehicles.column("#2", width=130)
            tree_vehicles.column("#3", width=90)
            tree_vehicles.column("#4", width=100)
            tree_vehicles.column("#5", width=130)
            tree_vehicles.column("#6", width=110)

            tree_vehicles.grid(row=1, column=0, rowspan=3, sticky='nsew')

            load_vehicles()


        def renew_vehicle_ui():
            ...
            def renew():
                try:
                    ...
                    if not is_valid_plate_number(plate_number):
                        raise Exception(messagebox.showerror("Invalid Plate Number", "Enter a valid plate number."))

                    if not check_plate_number(plate_number):
                        raise Exception(messagebox.showerror("Invalid Plate Number", f"{plate_number} is not registered."))

                    if renew_vehicle(plate_number, expiration_date):
                        messagebox.showinfo("Success", "Vehicle renewed")

                except Exception as e:
                    print(f"Error Occurred! {e}")

            tk.Frame(self.page_vehicles, bg='#e6e6e6', height=20).grid(row=1, column=0, sticky='nsew')
            frame_renew_vehicle = tk.Frame(self.page_vehicles, bg='#b80000', padx=10, pady=10, width=500)
            frame_renew_vehicle.grid(row=2, column=0, sticky='nsew')

            tk.Frame(frame_renew_vehicle, bg='#b80000', height=50).grid(row=0, column=0, sticky='nsew')

            label_renew_vehicle = tk.Label(frame_renew_vehicle, text="Renew Vehicle",
                                              font=self.master.header_font,
                                              bg='#b80000', fg='#ffffff')
            label_renew_vehicle.place(x=0, y=0)

            label_plate_number = tk.Label(frame_renew_vehicle, text="Plate Number", bg='#b80000', fg="white",
                                          font=self.master.subheader_font)
            label_plate_number.grid(row=1, column=0, sticky='w')

            entry_plate_number = tk.Entry(frame_renew_vehicle, bg='#b80000', fg="white", relief="flat")
            entry_plate_number.grid(row=2, column=0, sticky='w')

            tk.Canvas(frame_renew_vehicle, bg="white", height=0, width=100,
                      highlightthickness=0).grid(row=3, column=0, sticky='nsew')

            tk.Frame(frame_renew_vehicle, bg='#b80000', height=15).grid(row=4, column=0, sticky='nsew')

            label_valid_till = tk.Label(frame_renew_vehicle, text="Valid Until", bg='#b80000', fg="white",
                                        font=self.master.subheader_font)
            label_valid_till.grid(row=5, column=0, sticky='w')

            entry_valid_till = DateEntry(frame_renew_vehicle, width=12, takefocus=False, background='#b80000',
                                         style="Calendar.TEntry", relief="flat", year=2025,
                                         date_pattern="mm/dd/yyyy", font=self.master.login_font)
            entry_valid_till.grid(row=6, column=0, columnspan=3, sticky='nsew')

            label_date_format = tk.Label(frame_renew_vehicle, text="MM/DD/YYYY", bg="#b80000", fg="white")
            label_date_format.grid(row=6, column=2, sticky='e', padx=3, pady=10)

            tk.Canvas(frame_renew_vehicle, bg="white", height=0, width=300, highlightthickness=0).grid(row=7,
                                                                                                          column=0,
                                                                                                          columnspan=3,
                                                                                                          sticky='nsew')

            tk.Frame(frame_renew_vehicle, bg='#b80000', height=25).grid(row=8, column=0, sticky='nsew')

            button_register_vehicle = tk.Button(frame_renew_vehicle, text="Renew Vehicle", bg='#ffcc00',
                                                fg='black',
                                                relief="flat", command=renew, font=self.master.subheader_font)
            button_register_vehicle.grid(row=9, column=0, columnspan=3, sticky='nsew')
